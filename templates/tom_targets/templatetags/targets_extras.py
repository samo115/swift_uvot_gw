...
@register.inclusion_tag('tom_targets/partials/recent_groups.html', takes_context=True)
def recent_groups(context, limit=10):
    """
    Displays a list of the most recently created groups in the TOM up to the given limit, or 10 if not specified.
    """
    user = context['request'].user
    return {'targetgroups': get_objects_for_user(user, 'tom_targetgrouping.view_groups').order_by('-created')[:limit]}

@register.inclusion_tag('tom_targets/partials/recent_targets.html', takes_context=True)
def recent_target(context, limit=10):
    """
